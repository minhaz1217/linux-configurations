...
func resetBadRequestCountEndPoint(w http.ResponseWriter, r *http.Request) {
	...
	output := fmt.Sprintf("%d", hitCount)
	outputLog := fmt.Sprintf("%sresetBadRequestCountEndPoint Got hit from: %s Output: %s", version, r.URL.Path[1:], output)
	fmt.Println(outputLog)
	fmt.Fprintf(w, output)
}
...
func delayEndPoint(w http.ResponseWriter, r *http.Request) {
	...
	outputLog := fmt.Sprintf("delay Got hit from: %s Output: %s", r.URL.Path[1:], output)
	fmt.Println(outputLog)
	if err != nil {
		fmt.Fprintf(w, fmt.Sprintf("%s%s\n",version, "ERROR"))
		return
